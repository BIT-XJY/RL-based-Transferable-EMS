...
class DDPG(object):
    ...
    def OrnsteinUhlenbeckActionNoise(self, mu, sigma, theta=.15, dt=1e-2):
        ou_noise = self.ou_noise_prev + theta * (mu - self.ou_noise_prev) * dt + sigma * np.sqrt(dt) * np.random.normal(
            mu, sigma)
    ...
    def choose_action(self, s, loop, param_noise_scale, param_noise=True, action_noise_type=None):
        ...
        if action_noise_type == 'gs':
            ...
            output_noise = output + self.OrnsteinUhlenbeckActionNoise(0, 0.2, 0.15, 0.01)  # OU noise
    ...
    def savemodel(self):
        ...
        self.saver.save(self.sess, 'Checkpoints/target/save_net.ckpt', global_step=step_episode)
...
for i in range(MAX_EPISODES):
    path = "Data_Standard Driving Cycles/Prius_target_data"
    ...
    s = np.zeros(s_dim)
    s[0] = car_spd / 27.0756
    s[1] = (car_a - (-1.4402)) / (1.8671 - (-1.4402))
    s[2] = SOC
    ...
    for j in range(car_spd_one.shape[1] - 1):
        ...
        s_[0] = car_spd / 27.0756  # velocity
        s_[1] = (car_a - (-1.4402)) / (1.8671 - (-1.4402))  # acceleration
        s_[2] = SOC_new  # soc
        ...
        if j == (car_spd_one.shape[1] - 2):
            ...
            print('Episode:', i, ' cost_Engine: %.3f' % cost_Engine, ' reward: %.3f' % -(ep_reward_all / 100),
                  ' SOC-final: %.3f' % SOC)

    DDPG.savemodel()

    if not os.path.exists("./results_transfer"):
        os.mkdir("./results_transfer")
    SOC_final_arr = np.array(SOC_final_list)
    np.savetxt('./results_transfer/soc.txt', SOC_final_arr)
    cost_Engine_arr = np.array(cost_Engine_list)
    np.savetxt('./results_transfer/cost_Engine.txt', cost_Engine_arr)
    cost_all_arr = np.array(cost_all_list)
    np.savetxt('./results_transfer/cost_all.txt', cost_all_arr)
    mean_reward_arr = np.array(mean_reward_list)
    np.savetxt('./results_transfer/mean_reward.txt', mean_reward_arr)
    std_reward_arr = np.array(std_reward_list)
    np.savetxt('./results_transfer/std_reward.txt', std_reward_arr)
